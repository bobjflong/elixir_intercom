defmodule IntercomJavascriptTest do
  use ExUnit.Case
  alias ESTree.Tools.Generator
  doctest IntercomJavascript

  test "turning maps into JavaScript objects" do
    ast = IntercomJavascript.to_javascript_object(%{:foo => "bar"})
    code = Generator.generate(ast)
    assert code == "{\n        foo: 'bar'\n  }"
  end

  test "Intercom boot" do
    ast = IntercomJavascript.boot(%{:email => "bob@bob.com"})
    code = Generator.generate(ast)
  ...
  test "Secure mode with no secret" do
    ...
    with_hash = IntercomJavascript.inject_user_hash(nil, opts)
    assert with_hash == opts
  end

  test "Secure mode with secret and email" do
    opts = %{:email => "bob@bob.com"}
    with_hash = IntercomJavascript.inject_user_hash("foo", opts)
  ...
  test "full snippet" do
    opts = %{:email => "bob@bob.com"}
    snippet = IntercomJavascript.snippet("foo", opts)
